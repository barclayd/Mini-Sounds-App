//
//  BBCSMPSelectorPlayerItemProvider.m
//  BBCMediaPlayer
//
...
@interface BBCSMPMediaSelectorPlayerItemProvider ()
...
@implementation BBCSMPMediaSelectorPlayerItemProvider {
...
- (instancetype)initWithMediaSelectorClient:(BBCMediaSelectorClient*)mediaSelectorClient mediaSet:(NSString*)mediaSet vpid:(NSString*)vpid artworkFetcher:(id<BBCSMPArtworkFetcher>)artworkFetcher
{
    return [self initWithMediaSelectorClient:mediaSelectorClient
                                    mediaSet:mediaSet
                                        vpid:vpid
                              artworkFetcher:artworkFetcher
                                   blacklist:[[BBCSMPPermanentMediaURLBlacklist alloc] init]];
}

- (instancetype)initWithMediaSelectorClient:(BBCMediaSelectorClient*)mediaSelectorClient mediaSet:(NSString*)mediaSet vpid:(NSString*)vpid
{
    return [self initWithMediaSelectorClient:mediaSelectorClient mediaSet:mediaSet vpid:vpid artworkFetcher:nil];
}

- (instancetype)initWithMediaSet:(NSString*)mediaSet vpid:(NSString*)vpid
{
    return [self initWithMediaSelectorClient:[BBCMediaSelectorClient sharedClient] mediaSet:mediaSet vpid:vpid];
}
...
- (void)requestPlayerItem:(BBCSMPItemProviderSuccess)success failure:(BBCSMPItemProviderFailure)failure
{
    ...
    [_mediaSelectorClient sendMediaSelectorRequest:request
        success:^(BBCMediaSelectorResponse* response) {
            ...
            [[BBCSMPMediaSelectorPlayerItemProvider logger] logMessage:message];
            ...
            if([playerItem isPlayable]) {
                [self logWeAreAttemptingToPlayPlayableItem:playerItem];
                 ...
                 NSError* adapterError = [NSError errorWithDomain:@"smp-ios" code:1052 userInfo:@{ NSLocalizedDescriptionKey : @"Media Resolution Failed" }];
...
- (NSArray*)connections
{
    ...
    if(_suppliers)
    {
        ...
        for (NSString *supplier in _suppliers) {
            [requiredSuppliers addObjectsFromArray:[BBCSMPMediaSelectorPlayerItemProvider supplierDictionary][supplier]];
        }
...
- (BBCSMPMediaSelectorItem *)playerItemForCurrentConnection
{
    ...
    playerItem.actionOnBackground = BBCSMPBackgroundActionDefault;
